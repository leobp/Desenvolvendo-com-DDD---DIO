...
namespace Infra.Shared.Mapper
{
    public static class WorkerMapper
    {
        public static Worker ConvertToUserEntity(this CreateWorkerModel userModel) =>
            new Worker(0, userModel.Name, userModel.BirthDate ,userModel.Salary , userModel.Nin);
        ...
        public static IEnumerable<WorkerModel> ConvertToUsers(this IList<Worker> users) =>
            new List<WorkerModel>(users.Select(s => new WorkerModel(s.Id, s.Name.ToString(), s.BirthDate, s.Nin.ToString(), s.Salary.ToString())));

        public static WorkerModel ConvertToUser(this Worker user) =>
            new WorkerModel(user.Id, user.Name.ToString(), user.BirthDate, user.Nin.ToString(), user.Salary.ToString());
    }
}
